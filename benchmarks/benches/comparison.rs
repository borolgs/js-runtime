...
default_context!(Context, RinjaContext, QuickJsContext);
...
pub fn bench_compare_render(c: &mut Criterion) {
    ...
    g.bench_function("quickjs", |b| {
        js::Runtime::new(RuntimeConfig {
            js_src: Some(include_dir::include_dir!(
                "$CARGO_MANIFEST_DIR/inputs/comparison"
            )),
            ..Default::default()
        });

        let (context, _) = js::context::init(HashMap::new()).unwrap();

        context
            .eval_module(
                r#"import { Page } from "./quickjs.jsx";globalThis.Page = Page;"#,
                false,
            )
            .unwrap();
